...
// VERSI FINAL 2: Memperbaiki logika parsing untuk respons streaming berbentuk array.
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // PERBAIKAN UTAMA: Langsung parse sebagai JSON dan proses array-nya.
        const result = await response.json();
        
        let fullAnswer = "";

        if (Array.isArray(result)) {
            // Logika baru untuk menangani respons berbentuk array
            for (const chunk of result) {
             ...
             // Logika lama sebagai fallback jika formatnya kembali seperti semula
